...
function setupFilters() {
  ...
  if (!currentFilter) {
    ...
    const sv = localStorage.getItem('albasem_cat');
  if (!currentFilter) currentFilter = (sv === 'FAVORITES' || (sv && rawAreas.includes(sv))) ? sv : (rawAreas[0] || '');
  localStorage.setItem('albasem_cat', currentFilter);
  filterBox.innerHTML = '';
  const fb = document.createElement('button');
  fb.className = 'filter-chip flex-shrink-0 px-3.5 py-1.5 rounded-full border text-xs font-bold whitespace-nowrap ' + (currentFilter === 'FAVORITES' ? 'bg-amber-500 text-slate-950 border-amber-400' : 'bg-slate-900 text-amber-400 border-amber-500/30');
...
function renderCams() {
  ...
  let filtered = [];
  if (currentFilter === 'FAVORITES') {
    const f = window.getFavorites(); filtered = streamsData.filter(x => f.includes(String(x.id)));
    if (!filtered.length) { grid.innerHTML = '<div class="col-span-full py-16 text-center text-slate-400 text-xs">قائمة المفضلة فارغة حالياً ⭐ اضغط على أيقونة النجمة بأي قناة لحفظها هنا.</div>'; return; }
  } else if (currentFilter === 'all') { filtered = streamsData.filter(x => x.area !== 'IPTV'); }
  else { filtered = streamsData.filter(x => x.area === currentFilter); }
...
function openModal(streamId) {
  ...
  const box = document.getElementById('modal-content'); box.innerHTML = '';
  const url = stream.url || '';
  const isMp4 = url.toLowerCase().includes('.mp4') || (!url.toLowerCase().includes('.m3u8') && !url.includes('manifest') && !url.includes('youtu'));

  if (stream.type === 'youtube' || url.includes('youtu')) {
    let y = url; const m = y.match(/(?:v=|\/embed\/|youtu\.be\/)([\w-]{11})/);
    if (m) y = 'https://www.youtube-nocookie.com/embed/' + m[1];
    box.innerHTML = '<iframe class="w-full h-full border-0" src="' + y + '?autoplay=1&controls=1" allowfullscreen></iframe>';
  } else if (isMp4) {
    const v = document.createElement('video'); v.className = 'w-full h-full object-contain'; v.src = url; v.controls = true; v.autoplay = true; box.appendChild(v);
    v.play().catch(()=>{ v.muted = true; v.play().catch(()=>{}); });
  } else {
    launchHlsStream(box, url, true, stream.area === 'IPTV');
  }
