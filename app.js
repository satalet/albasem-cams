...
function toggleFavorite(id, e) {
  ...
  const sid = String(id);
  const exists = favs.includes(sid);
  favs = exists ? favs.filter(x => x !== sid) : [...favs, sid];
  localStorage.setItem('albasem_favs', JSON.stringify(favs));
  if (typeof currentFilter !== 'undefined' && currentFilter === 'FAVORITES') {
    renderCams();
  } else {
    document.querySelectorAll('.fav-btn-' + sid).forEach(btn => {
      const isNowFav = !exists;
      btn.className = 'fav-btn fav-btn-' + sid + ' p-1.5 rounded-lg flex items-center justify-center text-xs transition active:scale-90 ' + (isNowFav ? 'text-amber-400 bg-amber-400/10' : 'text-slate-400 hover:text-amber-400 bg-slate-800/60');
...
function launchHlsStream(container, url, isModal = false, isIptv = false) {
  ...
  // حل مشكلة تعذر العرض المباشر: أي رابط IPTV أو http على الموبايل يمر فوراً عبر الوسيط
...
function openModal(streamId) {
  ...
  if (isYt) {
    ...
    // تمرير خيار IPTV بشكل صحيح للمودال لتشغيل نفس القناة الشغالة بالكرت
    const isIptvStream = stream.area === 'IPTV' || (stream.url && stream.url.startsWith('http://'));
    const vid = launchHlsStream(modalBox, stream.url, true, isIptvStream);
