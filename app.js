...
function launchHlsStream(container, url, isModal = false, isIptv = false) {
  const isMp4Video = url && (url.toLowerCase().includes('.mp4') || (!url.toLowerCase().includes('.m3u8') && !url.includes('manifest') && !url.includes('youtu')));
  if (isMp4Video) {
    container.innerHTML = '';
...
function renderCams() {
  ...
  filtered.forEach(stream => {
    ...
    card.onclick = (e) => {
      if (!e.target.closest('button, input, label, a, .btn-inline-play')) {
        openModal(stream.id);
      }
    };
    ...
    const header = `
      <div onclick="openModal('${stream.id}')" class="px-3 py-2 bg-[#121c33] border-b border-slate-800/80 flex justify-between items-center text-xs cursor-pointer hover:bg-slate-800/60 transition select-none">
        ...
        <div class="flex items-center gap-1.5 flex-shrink-0">
          ${adminActions}
          <button onclick="toggleFavorite('${stream.id}', event)" class="fav-btn fav-btn-${stream.id} p-1 rounded flex items-center justify-center text-xs transition active:scale-90${window.getFavorites().includes(String(stream.id)) ? 'text-amber-400 bg-amber-400/10' : 'text-slate-400 hover:text-amber-400 bg-slate-800/60'}" title="المفضلة">
            <i class="${window.getFavorites().includes(String(stream.id)) ? 'fa-solid text-amber-400' : 'fa-regular text-slate-400'} fa-star"></i>
          </button>
