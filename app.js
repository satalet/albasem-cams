...
// كتم قسري لجميع شاشات العرض بالخلفية
function muteAllGridVideos() {
  document.querySelectorAll('#cams-grid video, #cams-grid iframe').forEach(v => {
    try {
      v.muted = true;
      v.volume = 0;
...
window.getFavorites = () => {
  try { return JSON.parse(localStorage.getItem('albasem_favs') || '[]'); } catch(e) { return []; }
};
window.toggleFavorite = (id, e) => {
  if (e) { e.stopPropagation(); e.preventDefault(); }
  let favs = window.getFavorites(), sid = String(id);
  favs = favs.includes(sid) ? favs.filter(x => x !== sid) : [...favs, sid];
  localStorage.setItem('albasem_favs', JSON.stringify(favs));
  renderCams();
};
...
function launchHlsStream(container, url, isModal = false, isIptv = false) {
  if (!url) return null;
  const isDirectMp4 = url.toLowerCase().includes('.mp4') || url.toLowerCase().includes('.webm') || (!url.toLowerCase().includes('.m3u8') && !url.includes('manifest') && !url.includes('youtu'));

  if (isDirectMp4) {
    container.innerHTML = '';
    const v = document.createElement('video');
    v.className = isModal ? 'w-full h-full object-contain' : 'absolute inset-0 w-full h-full object-cover pointer-events-none';
    v.src = url;
    v.controls = isModal; // في الشبكة بدون تحكم لتسهيل اللمس ولعدم تشغيل الصوت بالخطأ
    v.autoplay = true;
    v.loop = true;
    v.playsInline = true;
    
    // كتم تام في الشبكة، وصوت فقط في المودال
    if (isModal) {
      v.muted = false;
      v.volume = 1.0;
    } else {
    ...
    // إخفاء دائرة "جاري فتح البث..." فور بدء العرض
    v.onloadeddata = () => {
      const loader = document.getElementById('modal-loader') || document.querySelector('.modal-loading');
  ...
  video.className = (isModal || isIptv) ? 'w-full h-full object-contain' : 'absolute inset-0 w-full h-full object-cover pointer-events-none';
  video.autoplay = true;
...
function openModal(streamId) {
  const stream = typeof streamId === 'object' ? streamId : streamsData.find(s => s.id === streamId);
  if (!stream) return;

  // 1. كتم كل أصوات قنوات الخلفية فوراً
  if (window.muteAllGridVideos) window.muteAllGridVideos();

  document.getElementById('modal-title').textContent = (stream.title || 'بث مباشر') + ' - ' + (stream.area || '');
  const modalBox = document.getElementById('modal-content');
  modalBox.innerHTML = '';

  // 2. إخفاء رسالة الخطأ السابقة وإظهار اللودر مؤقتاً
  const errBox = document.getElementById('modal-error');
  if (errBox) errBox.classList.add('hidden');
  const loader = document.getElementById('modal-loader') || document.querySelector('.modal-loading');
  if (loader) loader.classList.remove('hidden');

  const isYt = stream.type === 'youtube' || (stream.url && (stream.url.includes('youtube') || stream.url.includes('youtu.be')));
  const isDirectMp4 = stream.url && (stream.url.toLowerCase().includes('.mp4') || (!stream.url.toLowerCase().includes('.m3u8') && !stream.url.includes('manifest')));

  if (isYt) {
    let ytUrl = stream.url;
    if (!ytUrl.includes('/embed/')) {
      const match = ytUrl.match(/(?:v=|\/embed\/|youtu\.be\/)([\w-]{11})/);
      if (match) ytUrl = 'https://www.youtube-nocookie.com/embed/' + match[1];
    }
    if (loader) loader.classList.add('hidden');
    ...
    launchHlsStream(modalBox, stream.url, true, false);
  } else {
...
function closeModal(fromHistory = false) {
  ...
  if (modal && !modal.classList.contains('hidden')) {
    ...
    // 2. إيقاف وتفريغ وحذف أي ميديا في المودال لقتل الصوت نهائياً
    const modalBox = document.getElementById('modal-content');
    if (modalBox) {
    ...
    // 3. تأكيد كتم جميع فيديوهات الخلفية
    if (window.muteAllGridVideos) window.muteAllGridVideos();

    // 4. إخفاء المودال وعناصر التحميل
    const errBox = document.getElementById('modal-error');
    if (errBox) errBox.classList.add('hidden');
    const loader = document.getElementById('modal-loader') || document.querySelector('.modal-loading');
    if (loader) loader.classList.add('hidden');
