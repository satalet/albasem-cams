...
// ==================== محرك الفرز والترحيل الجماعي المعتمد ====================
let isBulkSortActive = false;
let selectedCamKeys = new Set();

window.toggleBulkSortMode = function() {
    isBulkSortActive = !isBulkSortActive;
    const bar = document.getElementById('bulkActionBar');
    if (bar) {
        if (isBulkSortActive) bar.classList.remove('hidden');
        else bar.classList.add('hidden');
    }
    
    // إظهار أو إخفاء مربعات التحديد بكل الكروت
    document.querySelectorAll('.bulk-chk-label').forEach(el => {
        if (isBulkSortActive) el.classList.remove('hidden');
...
window.toggleSelectCard = function(id, isChecked) {
    if (isChecked) selectedCamKeys.add(id);
    else selectedCamKeys.delete(id);
...
window.selectAllBulk = function(select) {
    document.querySelectorAll('.bulk-stream-chk').forEach(cb => {
        cb.checked = select;
        if (select) selectedCamKeys.add(cb.value);
    });
    if (!select) selectedCamKeys.clear();
    const countLbl = document.getElementById('bulkSelectedCount');
    if (countLbl) countLbl.innerText = selectedCamKeys.size;
};

window.executeBulkMove = async function() {
    if (selectedCamKeys.size === 0) return alert('⚠️ يرجى تحديد قناة واحدة على الأقل!');
    const target = document.getElementById('bulkTargetArea').value;
    if (!target) return alert('⚠️ يرجى اختيار القسم المستهدف أولاً!');

    if (!confirm(`هل أنت متأكد من ترحيل (${selectedCamKeys.size}) قنوات إلى قسم [${target}]؟`)) return;

    try {
        const updates = {};
        selectedCamKeys.forEach(id => {
...
window.executeBulkDelete = async function() {
    if (selectedCamKeys.size === 0) return alert('⚠️ يرجى تحديد القنوات المراد حذفها أولاً!');
    if (!confirm(`⚠️ تحذير: هل أنت متأكد من حذف (${selectedCamKeys.size}) قنوات نهائياً من الموقع؟`)) return;
  ...
  filtered.forEach(stream => {
    ...
    const adminActions = currentUser ? `
      <div class="flex items-center gap-1.5 ml-2 border-l border-slate-700 pl-2" onclick="event.stopPropagation()">
        <label class="bulk-chk-label ${isBulkSortActive ? 'flex' : 'hidden'} items-center gap-1 text-[11px] text-emerald-400 bg-emerald-950/70 border border-emerald-500/50 px-1.5 py-0.5 rounded cursor-pointer select-none">
          <input type="checkbox" class="bulk-stream-chk accent-emerald-500 cursor-pointer w-3.5 h-3.5" value="${stream.id}" onchange="toggleSelectCard('${stream.id}', this.checked)" ${selectedCamKeys.has(stream.id) ? 'checked' : ''}>
          <span>تحديد</span>
        </label>
